...
class FakeTesseract:
    ...
    def __init__(self, model, tessdata_path=None, character_whitelists=None) -> None:
        self.model = model
        self.character_whitelists = character_whitelists
        self.closed = False
...
def test_tesseract_pool_reuses_bounded_engine_instances(monkeypatch) -> None:
    ...
    pool = tesseract_engine.PooledTesseractEngine(
        "model", workers=2, character_whitelists={"a": "0123"}
    )
    ...
    assert all(
        instance.character_whitelists == {"a": "0123"}
        for instance in FakeTesseract.instances
    )
